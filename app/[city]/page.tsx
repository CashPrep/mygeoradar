...
// Guard: prevent city route from catching industry slugs
// (Next.js picks the more specific segment first, but this is a safety net)
const INDUSTRY_SLUGS = new Set([
  'dentists','plumbers','lawyers','restaurants','med-spas',
  'chiropractors','contractors','real-estate-agents',
])

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  if (INDUSTRY_SLUGS.has(params.city)) return {}
  ...
  return {
    title,
    description,
    openGraph: { title, description, url: `https://mygeoradar.com/${params.city}` },
    alternates: { canonical: `https://mygeoradar.com/${params.city}` },
  }
}
...
const FAQS = [
  {
    ...
    a: 'The most common reasons are an incomplete or unverified Google Business Profile, inconsistent name/address/phone data across directories, and missing schema markup on your website. Any one of these can suppress your local rankings — the free scan checks all three in 30 seconds.',
  },
  {
    q: 'Does my {city} business need to appear in AI search results?',
    a: 'Yes — AI tools like ChatGPT, Perplexity, and Google Gemini are increasingly the first stop for local business searches. They recommend businesses by name based on structured data, directory citations, and authority signals. If your signals are weak, you won’t appear even if you have strong reviews.',
  },
  {
    q: 'How competitive is local SEO in {city}?',
    a: 'In most major metros, the businesses that rank are not necessarily the best — they’re the ones with the most complete visibility signals. A fully optimized GBP, clean citations, and proper schema markup can move a business from page 3 to the local map pack within weeks.',
  },
  {
    q: 'What does the free visibility scan check?',
    a: 'The scan evaluates your Google Business Profile completeness, citation consistency across directories, schema markup presence, and AI search signals. You get a scored breakdown of each layer with specific findings — not vague grades.',
  },
]

export default function CityHubPage({ params }: Props) {
  if (INDUSTRY_SLUGS.has(params.city)) notFound()
  const city = getCity(params.city)
  if (!city) notFound()
  ...
  const jsonLd = {
    ...
    url: `https://mygeoradar.com/${params.city}`,
    mainEntity: {
      '@type': 'FAQPage',
  ...
  return (
    <main className="min-h-screen bg-background">
      ...
      <section className="pt-28 pb-16 px-4 md:px-8 border-b border-border">
        <div className="max-w-3xl mx-auto">
          ...
          <p className="text-lg text-muted leading-relaxed mb-8 max-w-2xl">
            In {city.metro}, customers search Google, check AI assistants, and browse directories
            before contacting a local business. The free scan shows you exactly which visibility
            gaps are costing you customers right now.
          </p>
      ...
      <section className="py-16 px-4 md:px-8 border-b border-border">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-2xl font-bold mb-2">What the scan checks for {city.label} businesses</h2>
          <p className="text-muted text-sm mb-8">Four layers determine whether your business gets found — or gets passed over.</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[
              { icon: MapPin, label: 'Google Business Profile', desc: `Complete, verified, and optimized for ${city.label} local map pack ranking.` },
              { icon: Search, label: 'Directory Citations', desc: `Consistent NAP across Yelp, Apple Maps, Bing, and 40+ directories serving the ${city.metro} area.` },
              ...
              { icon: Zap, label: 'AI Search Visibility', desc: `Whether ChatGPT, Perplexity, and Gemini would recommend your ${city.label} business by name.` },
      ...
      <section className="py-16 px-4 md:px-8 border-b border-border">
        <div className="max-w-3xl mx-auto">
          ...
          <p className="text-muted leading-relaxed mb-6">
            ChatGPT, Perplexity, and Google Gemini now answer “best [business type] in {city.label}”
            with specific recommendations. They pull from structured data, directory citations, and
            authoritative mentions — not just star ratings. If your signals are incomplete, you’re
            invisible to this growing channel.
          </p>
      ...
      <section className="py-20 px-4 md:px-8">
        <div className="max-w-xl mx-auto text-center">
          ...
          <p className="text-muted mb-8">
            The free scan checks GBP, citations, schema, and AI signals — and tells you exactly
            what’s broken. No signup, no credit card, 30 seconds.
          </p>
          <Link href="/#scan">
